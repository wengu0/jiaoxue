...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($new_pwd != $comfirm_pwd) {
        ...
        if ($old_pwd != $pwd) {
            ...
            switch ($role) {
                case 1:
                    $sql = "update admin set admin_pwd='$new_pwd' where admin_id='$uname' and admin_pwd='$old_pwd'";
                    break;
                case 2:
                    $sql = "update teacher set tch_pwd='$new_pwd' where tch_name='$uname' and tch_pwd='$old_pwd'";
                    break;
                case 3:
                    $sql = "update student set stu_pwd='$new_pwd' where stu_name='$uname' and stu_pwd='$old_pwd'";
                    break;
            }
...
<body>
    <form action="update_pwd.php" method="post">
        <table width="500" border="1" height="300" align="center">
            ...
            <tr>
                <td>用户名:</td>
                <td><input type="text" name="uname" readonly value='<?php echo $uname; ?>'>
                    <input type="hidden" name="role" value='<?php echo $role; ?>'>
                </td>
            </tr>
            <tr>
                <td>原密码:</td>
                <td><input type="text" name="old_pwd"></input></td>
            </tr>
            <tr>
                <td>新密码:</td>
                <td><input type="text" name="new_pwd"></input></td>
            </tr>
            <tr>
                <td>确认密码:</td>
                <td><input type="text" name="comfirm_pwd"></input></td>
            </tr>
            <tr>
                <td></td>
                <td id="td1"><input class="btn btn-sm btn-success" type="submit"></input>&nbsp;&nbsp;&nbsp;<input class="btn btn-sm btn-secondary" type="button" value="返回" onclick="top.location.href='index.php'"></input></td>
            </tr>
        </table>
    </form>
</body>
